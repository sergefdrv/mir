...
// SendEvents notifies the simulation about a list of emitted events.
func (n *SimNode) SendEvents(proc *testsim.Process, eventList *events.EventList) {
	...
	for _, m := range moduleIDs {
		// XXX: This delay is a hack to ensure that event
		// lists destined to different modules are not
		// processed in the same simulation step.
		//
		// TODO: Clarify if and why this is still necessary.
		//proc.Delay(1)
		//fmt.Println("Node", n.id, "Sending", eventsMap[m].Slice())
		proc.Send(n.moduleChans[m], eventsMap[m])
		proc.Yield() // HACK: to ensure there is at most one active receiver
	}
}
...
func (n *SimNode) Start(proc *testsim.Process, walEvents *events.EventList /* , doneChan *testsim.Chan */) {
	//proc := n.Spawn()
	// go func() {
	n.SendEvents(proc, walEvents)
	//proc.Delay(1) // TODO: do we still need this hack?
	for m := range n.moduleChans {
		...
		//proc.Yield() // TODO: do we still need this hack?
	}
	// proc.Send(doneChan, nil)
	//proc.Exit()
	// }()
}
...
func (m *simModule) run(proc *testsim.Process, applyFn applyEventsFn) {
	...
	for {
		...
		//fmt.Println("Node", m.id, "Received", origEvents.Slice())

		in := <-m.inChan

		for origEvents.Len() < in.eventList.Len() {
			//fmt.Println("Node", m.id, "Waiting for missing events", origEvents.Slice(), in.eventList.Slice())
			newOrigEvents, ok := m.SimNode.recvEvents(proc, m.simChan)
			if !ok {
				return
			}
		...
		for e := it.Next(); e != nil; e = it.Next() {
			...
			d := m.SimNode.delayFn(e)
			//fmt.Println("Node", m.id, "Sleeping for", d, "before applying", in.eventList.Slice())
			if !proc.Delay(d) {
				return
			}
		}

		//fmt.Println("Node", m.id, "Applying", in.eventList.Slice())

		var out eventsOut
		out.eventList, out.err = applyFn(in.ctx, in.eventList)

		//fmt.Println("Node", m.id, "Applied", in.eventList.Slice())
		...
		if out.err == nil {
			...
			plainEvents, followUps := origEvents.StripFollowUps()
			_ = plainEvents
			// if !cmp.Equal(in.eventList, plainEvents, cmpopts.IgnoreUnexported(eventpb.Event{})) {
			// 	panic(cmp.Diff(in.eventList, plainEvents, cmpopts.IgnoreUnexported(eventpb.Event{})))
			// }
			...
			proc.Yield() // HACK: to prevent multiple modules from handling events concurrently
		}
	}
}
...
func (m *simModule) applyEvents(ctx context.Context, eventList *events.EventList) (eventsOut *events.EventList, err error) {
	...
	case <-time.After(1 * time.Second):
		//fmt.Println("Node", m.id, "Got stuck applying", eventList.Slice())
		select {}
	}
}
